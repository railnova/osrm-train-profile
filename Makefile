...
output/filtered.osrm: output/filtered.osm.pbf new.lua
	docker run -t -v $(shell pwd):/opt/host osrm/osrm-backend:v5.20.0 osrm-extract -p /opt/host/new.lua /opt/host/$<
...
serve: output/filtered.osrm new.lua
	docker run -t -i -p 5000:5000 -v $(shell pwd):/opt/host osrm/osrm-backend:v5.20.0 osrm-routed --algorithm mld /opt/host/$<
